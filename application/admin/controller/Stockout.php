...
class Stockout extends Admin{
    private $model_stock_out, $model_stock;
    protected function _initialize(){
        parent::_initialize();
        $this->model_stock_out = model('common/StockOut');
    ...
    public function index(){
        if(request()->isGet()){
            ...
            if(isset($post_data['kw'])){
                $map['out_remarks'] = array('like', '%'.$post_data['kw'].'%');
            }
            if($post_data['goods_id'] != ''){
                $map['goods_id'] = $post_data['goods_id'] ? $post_data['goods_id'] : 0;;
            }
            $map['send_shop_id'] = $this->shop_id;
            $field = '*';
            $count = $this->model_stock_out->where($map)->count();
            $flows = $this->model_stock_out->where($map)->order('create_time DESC,st_outid DESC')->field($field)->select();
    ...
    public function add(){
        if(request()->isPost()){
            $data = input('post.');
            $data['send_shop_id'] = $this->shop_id;
            $data['send_shop_group_id'] = $this->shop_group_id;
            $stock_info = $this->set_stock($data);
            if($stock_info['status'] < 0){
                return json_encode(array('status'=> -888, 'msg'=>$stock_info['msg']));
            }
            $result = $this->model_stock_out->validate(true)->allowField(true)->save($data);
            if ($result !== false) {
                return json_encode(array('status'=> 0));
            }else {
                return json_encode(array('status'=> -888, 'msg'=>$this->model_stock_out->getError()));
            }
        }else{
            $info = array();
    ...
    public function edit($id = 0){
        if(request()->isPost()){
            $data = input('post.');
            $this->is_your_infor($data['st_outid']);
            $result = $this->model_stock_out->validate(true)->allowField(true)->save($data, array('st_outid' => $data['st_outid']));
            if ($result !== false) {
                return json_encode(array('status'=> 0));
            }else {
                return json_encode(array('status'=> -888, 'msg'=>$this->model_stock_out->getError()));
            ...
            $info = array();
            /* 获取数据 */
            $info = $this->model_stock_out->field('*')->find($id);
            if ($info === false) {
    ...
    public function delete($id = 0){
        $this->is_your_infor($id);
        $result = $this->model_stock_out->where(array('st_outid' => $id))->delete();
//        $result =  $this->model_stock_out->where(array('st_outid' => $id))->setField('acc_is_status', -1);
        if ($result !== false) {
            return json_encode(array('status'=> 0));
        }else {
    ...
    private function is_your_infor($id = 0){
        $info = $this->model_stock_out->field('st_outid,accpet_shop_id')->find($id);
        if ($info === false) {
            if(request()->isGet()){
    ...
    private function set_stock($data){
        ...
        if($has){
            if($has->goods_num < $data['goods_num']){
                return array('status'=>-1, 'msg'=>'库存不足，目前只有'.$has->goods_num.'件'.'存货！');
            }
            $result =  $this->model_stock->where($map)->setField('goods_num', ($has->goods_num - $data['goods_num']));
            if($result){
                return array('status'=>1, 'msg'=>$this->model_stock->getError());
            }else{
                return array('status'=>-1, 'msg'=>$this->model_stock->getError());
            }
        }else{
            return array('status'=>-1, 'msg'=>'库存不足，目前只有0件'.'存货！');
        }
    }
}