...
;(function($, w) {
	...
	$.extend(Upload.prototype, {
		...
		initUploader: function() {
			...
			if (!self.options.uploadEvents.fileQueued) {
				self.BDUploader.on('fileQueued', function(file) {
					...
					if ($("#" + self.options.listName).length > 0) {
						...
						var closeHtml = '<span class="webuploader-pick-file-close" data-queued-id="' + file.id + '"><i class="layui-icon"></i></span>';
			...
			if (!self.options.uploadEvents.uploadSuccess) {
				self.BDUploader.on('uploadSuccess', function(file, response) {
					...
					if (type == 'image') {
						filebox_html = '<img width="200" src="' + SITE_URL + response.data.path + '" class="img-responsive" />';
					}else{
						filebox_html = '';
					}
					$fileItem.find('div.filebox').addClass(type).html(filebox_html);
					$fileItem.find('.upload-file-info span').eq(0).attr('data-id', response.data.aid).attr('data-fileurl', response.data.path);
					var responseVal = '';
					console.log(response);
					responseVal = self.options.hiddenValType == '1' ? response.data.aid : response.data.aid
		...
		delFile: function() {
				...
				$(selecter).on('click', '.webuploader-pick-file-close', function() {
					...
					if (fid > 0) {
						self.setHiddenValue(remove_array(fid, self.getHiddenValue().split(',')));
	...
	$.fn[pluginName].defaults = {
		...
		*/
		...
		swf: 'public/static/js/lib/webuploader/Uploader.swf',
		server: SITE_URL + '/api/upload/upload.html?flash=1', // 文件接收服务端。
		/**
		...
		delUrl: SITE_URL + '/api/upload/delete.html',
		separator: ',', //默认逗号
	}
