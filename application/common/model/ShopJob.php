...
class ShopJob extends Model{
     ...
     * @return void
     */
    public function get_order_worker_by_jobsn($job_sn='',$field='*',$is_orderinfo=false){
        //初始化
        $return = array();
        //条件查询
        if($job_sn){
            ...
            //获取sg_id组信息
            $server_group = $this->where($where=['job_sn'=>$job_sn])->group('sg_id')->column('sg_id,job_remarks,work_group_id');
            ...
            if($server_group){
                foreach($server_group as $key=>$val){
                    ...
                    $val['material_list'] = $this->get_shop_job_list($where=['job_sn'=>$job_sn,'sg_id'=>$val['sg_id'],'job_type'=>1]);
                    $val['service_list']  = $this->get_shop_job_list($where=['job_sn'=>$job_sn,'sg_id'=>$val['sg_id'],'job_type'=>0]);
     ...
     * @return void
     */
    public function get_order_jobs_by_jobsn($job_sn='',$field='*',$is_orderinfo=false){
        //初始化
        $return = array();
        //条件查询
        if($job_sn){
     ...
     * @return void
     */
    public function get_shop_job_list($where=[],$limit='',$order='job_id desc',$field='*'){
        //初始化
        $return = array();
        //条件查询
        if($where){
     ...
     * @return void
     */
    public function get_shop_job_info($where=array(),$field='*'){
        //初始化
        $return = array();
        //条件查询
        if($where){
    ...
    * @return void
    */
    public function shop_job_save($post_data=[],$where=[]){
        //初始化
        $return = $map = array();
    ...
    public function shop_check_job_saveall($post_data=[],$order_recp_data){
        ...
        if(!empty($post_data) && $order_recp_data){
            ...
            foreach ($post_data as $key => &$value) {
                //默认job的状态值
                $value['status'] = \app\api\controller\ShopJob::$job_status['need_recept'];
                //和接待订单关联id
                ...
                if($value['sg_id'] &&($value['job_sn'] || $value['carorder_id'])){
                    if($value['work_group_id']){
                        $value['confirm_id'] = get_model_data($model='shop_user_group',$id=$value['work_group_id'],$field='shop_user_id',$field_where='group_id');
                    }
                    if($value['job_id']){
