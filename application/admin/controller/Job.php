...
class Job extends Admin{
    ...
    protected function _initialize(){
        parent::_initialize();
        $this->model_job = model('admin/ShopJob');
    }
    ...
    public function index(){
        if(request()->isGet()){
            ...
            echo json_encode($this->model_job->getPageList());
        }
    }
    ...
    public function show_detail($id = 0){
        ...
        $detail_data = $this->model_job->showRecpDetail($id);
//        dump($detail_data);
...
//            $data['status'] = ($data['status'] == 'on') ? 1 : 0;
            $result = $this->model_job->validate(true)->save($data);
            if ($result !== false) {
                return json_encode(array('status'=> 0));
            }else {
                return json_encode(array('status'=> -888, 'msg'=>$this->model_job->getError()));
            }
        }else{
            $info = array();
            $this->get_users();
    ...
    public function edit($id = 0){
        if(request()->isPost()){
            ...
            $data['shop_id'] = $this->shop_id;
//            $data['status'] = ($data['status'] == 'on') ? 1 : 0;
            $result = $this->model_job->validate(true)->save($data, array('carorder_id' => $data['carorder_id']));
            if ($result !== false) {
                return json_encode(array('status'=> 0));
            }else {
                return json_encode(array('status'=> -888, 'msg'=>$this->model_job->getError()));
            ...
            $info = array();
            /* 获取数据 */
            $info = $this->model_job->field('*')->find($id);
            if ($info === false) {
...
//        $result = $this->model_menu->where(array('group_id' => $id))->delete();
        $result =  $this->model_job->where(array('carorder_id' => $id))->setField('order_status', -1);
        if ($result !== false) {
            return json_encode(array('status'=> 0));
        }else {
    ...
    private function is_your_infor($id = 0){
        $info = $this->model_job->field('carorder_id,shop_id')->find($id);
        if ($info === false) {
            if(request()->isGet()){
